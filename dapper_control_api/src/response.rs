...
impl RawDapResult {
    ...
    pub fn render(&self) -> String {
        truncate_response(self.to_string())
    }
...
pub fn truncate_response(json: String) -> String {
    if json.len() > MAX_RESPONSE_SIZE {
        let truncated = truncate_at_char_boundary(&json, MAX_RESPONSE_SIZE);
        let file_path = match save_to_temp_file(&json) {
            Ok(path) => path,
            Err(e) => {
...
fn save_to_temp_file(content: &str) -> anyhow::Result<String> {
    ...
    let path = temp_dir.join(format!("dapper-response-{}.json", timestamp));
